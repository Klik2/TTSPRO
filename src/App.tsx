...
import {
  ...
  Moon, Heart, Smartphone, Globe, Languages, MessageCircle, ArrowRight, 
  Users, Activity, Infinity
} from 'lucide-react';

// --- CONFIGURATION & DATABASE (SAMA SEPERTI SEBELUMNYA) ---
// ... (Bagian database suara dan konfigurasi tetap sama, saya ringkas biar tidak kepanjangan)
...
const LandingPage = ({ onStart, isDarkMode, setIsDarkMode }: { onStart: () => void, isDarkMode: boolean, setIsDarkMode: (v: boolean) => void }) => {
  const features = [
    { icon: <Zap className="w-6 h-6 text-yellow-500" />, title: "AI Super Cepat", desc: "Didukung oleh Gemini & ElevenLabs untuk hasil suara natural instan." },
    { icon: <Users className="w-6 h-6 text-blue-500" />, title: "30+ Model Suara", desc: "Pilihan lengkap pria & wanita, termasuk logat daerah (Jawa, Sunda, dll)." },
    { icon: <Activity className="w-6 h-6 text-red-500" />, title: "Kaya Emosi", desc: "Atur intonasi: marah, sedih, gembira, atau berbisik sesuka hati." },
    { icon: <Mic className="w-6 h-6 text-purple-500" />, title: "Voice Cloning", desc: "Rekam suaramu sendiri dan biarkan AI menirukan gayamu." },
    { icon: <Infinity className="w-6 h-6 text-green-500" />, title: "Tanpa Batas", desc: "Konversi teks ke suara sepuasnya tanpa batasan karakter harian." },
    { icon: <Download className="w-6 h-6 text-cyan-500" />, title: "Download Instan", desc: "Unduh hasil audio (WAV) langsung ke perangkatmu." },
    { icon: <Globe className="w-6 h-6 text-indigo-500" />, title: "Multi-Bahasa", desc: "Terjemahkan teks otomatis ke berbagai bahasa sebelum dikonversi." },
    { icon: <Sun className="w-6 h-6 text-orange-500" />, title: "Tampilan Adaptif", desc: "Nyaman di mata dengan dukungan Mode Gelap & Terang." },
  ];

  return (
    <div className={`min-h-screen flex flex-col ${isDarkMode ? 'bg-neutral-950 text-white' : 'bg-slate-50 text-slate-900'} transition-colors duration-500`}>
      {/* Navbar Landing */}
      <nav className="p-6 flex justify-between items-center max-w-6xl mx-auto w-full">
        <div className="flex items-center gap-2">
           ...
           <h1 className="font-bold text-xl tracking-tight">Te_eR™ to Speech</h1>
        </div>
        <button 
          onClick={() => setIsDarkMode(!isDarkMode)}
          className={`p-2 rounded-full border ${isDarkMode ? 'border-white/10 hover:bg-white/10' : 'border-slate-200 hover:bg-slate-100'}`}
        >
      ...
      {/* Hero Section */}
      <div className="flex-1 flex flex-col items-center justify-center text-center px-6 py-12 max-w-4xl mx-auto space-y-8">
        <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-xs font-bold border ${isDarkMode ? 'bg-lime-500/10 border-lime-500/30 text-lime-400' : 'bg-blue-100 border-blue-200 text-blue-700'}`}>
          <Sparkles className="w-3 h-3" />
          TEXT TO BACOT PRO EDITION
        </div>
        
        <h2 className="text-5xl md:text-7xl font-extrabold tracking-tight leading-tight">
          Ubah Teks Jadi <br/>
          <span className={`text-transparent bg-clip-text bg-gradient-to-r ${isDarkMode ? 'from-blue-400 via-purple-400 to-lime-400' : 'from-blue-600 via-purple-600 to-cyan-500'}`}>
            Suara Bernyawa
          </span>
        </h2>
        
        <p className={`text-lg md:text-xl max-w-2xl ${isDarkMode ? 'text-neutral-400' : 'text-slate-600'}`}>
          Platform Text-to-Speech tercanggih dengan dukungan logat daerah Indonesia, 
          kloning suara, dan emosi yang nyata. Gratis tanpa batasan!
        </p>

        <button 
          onClick={onStart}
          className={`group relative px-8 py-4 rounded-full text-lg font-bold flex items-center gap-3 transition-all hover:scale-105 active:scale-95 shadow-xl
            ${isDarkMode ? 'bg-white text-black hover:bg-neutral-200' : 'bg-blue-600 text-white hover:bg-blue-700 shadow-blue-500/30'}`}
        >
          Mulai Buat Suara
      ...
      {/* Features Grid */}
      <div className={`py-20 px-6 ${isDarkMode ? 'bg-neutral-900/50' : 'bg-white'}`}>
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {features.map((feature, idx) => (
            <div key={idx} className={`p-6 rounded-2xl border transition-all hover:-translate-y-1 hover:shadow-lg ${isDarkMode ? 'bg-neutral-900 border-white/5 hover:border-lime-500/30' : 'bg-slate-50 border-slate-100 hover:border-blue-200'}`}>
              <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-4 ${isDarkMode ? 'bg-neutral-800' : 'bg-white shadow-sm border border-slate-100'}`}>
                {feature.icon}
              </div>
              <h3 className="font-bold text-lg mb-2">{feature.title}</h3>
              <p className={`text-sm leading-relaxed ${isDarkMode ? 'text-neutral-400' : 'text-slate-500'}`}>
                {feature.desc}
              </p>
            </div>
          ))}
      ...
      {/* Footer Landing */}
      <div className={`py-8 text-center border-t ${isDarkMode ? 'border-white/5 text-neutral-500' : 'border-slate-200 text-slate-400'}`}>
        <p className="text-sm">© 2026 Te_eR™ to Speech. All rights reserved.</p>
      </div>
    </div>
  );
};
...
export default function App() {
  ...
  const [isDarkMode, setIsDarkMode] = useState(true); // Default Dark Mode agar keren
  ...
  // Jika masih di Landing Page, tampilkan Landing Page
  if (showLanding) {
    return <LandingPage onStart={() => setShowLanding(false)} isDarkMode={isDarkMode} setIsDarkMode={setIsDarkMode} />;
  }

  // ... (SISA LOGIC UTAMA SAMA SEPERTI FILE SEBELUMNYA) ...
  // ... Paste Logic useEffect, saveSettings, handleTranslate, Recorder, Generator dll di sini ...
  
  // Karena keterbatasan panjang teks, saya asumsikan Anda menyalin ulang LOGIC & RETURN 
  // dari file App.tsx sebelumnya dan menempelkannya di bawah ini.
  
  // PASTE KODE LAMA ANDA DI SINI MULAI DARI `const colors = { ...` 
  // SAMPAI PENUTUP `return ( ... )` dari komponen App lama.
  ...
  // Re-declare effects here for completeness if copying blindly
  useEffect(() => {
    ...
    if (savedClones) { try { setCustomClones(JSON.parse(savedClones)); } catch (e) {} }
    window.addEventListener('beforeinstallprompt', (e) => { e.preventDefault(); setDeferredPrompt(e); });
    const interval = setInterval(() => { setShowSpeakerIcon(prev => !prev); }, 3000);
  ...
  const handleInstallClick = () => {
    ...
    } else { alert("Fitur install tidak tersedia atau aplikasi sudah terinstall."); }
  };

  const handleTranslate = async () => {
    setIsLoading(true); setStatusMsg(null);
    try {
      ...
      const response = await fetchWithRetry(
        `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${apiKey}`,
        {
          ...
          body: JSON.stringify({ contents: [{ parts: [{ text: `Translate the following text to ${targetLangName}. Only return the translated text, no explanations:\n\n${text}` }] }] })
        }
      );
      const data = await response.json();
      const translatedText = data.candidates?.[0]?.content?.parts?.[0]?.text;
      if (translatedText) { setText(translatedText.trim()); setStatusMsg({ type: 'success', text: 'Teks berhasil diterjemahkan!' }); setIsSettingsOpen(false); } 
      else { throw new Error("Gagal mendapatkan respon terjemahan."); }
    } catch (e: any) { setStatusMsg({ type: 'error', text: `Gagal translate: ${e.message}` }); } finally { setIsLoading(false); }
  };
  ...
  const startRecording = async () => {
    try {
      ...
      recorder.onstop = () => {
        ...
        setRecordedUrl(URL.createObjectURL(blob)); setRecordedBlob(blob);
        stream.getTracks().forEach(track => track.stop());
      };
      recorder.start(); mediaRecorderRef.current = recorder; setIsRecording(true); setRecordingTime(0);
      timerRef.current = setInterval(() => { setRecordingTime(prev => prev + 1); }, 1000);
    } catch (err) { setStatusMsg({ type: 'error', text: 'Gagal mengakses mikrofon.' }); }
  };
  ...
  const saveClone = () => {
    if (!customCloneName.trim()) { setStatusMsg({ type: 'error', text: 'Beri nama untuk suara ini.' }); return; }
    const newClone = { name: `[REC] ${customCloneName}`, id: `clone_${Date.now()}` };
    const updatedClones = [newClone, ...customClones];
    setCustomClones(updatedClones); localStorage.setItem('gemini_clones', JSON.stringify(updatedClones));
    setRecordedUrl(null); setRecordedBlob(null); setCustomCloneName(""); setStatusMsg({ type: 'success', text: 'Voice Clone Tersimpan!' });
  };
  ...
  const handleAutoWrite = async () => {
    ...
    try {
      const response = await fetchWithRetry(
        ...
        {
          method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ contents: [{ parts: [{ text: "Buatkan skrip pendek (maksimal 3 kalimat) yang lucu atau puitis dalam Bahasa Indonesia untuk demo text-to-speech." }] }] })
        }
      );
      const data = await response.json();
      ...
      setText(randomScript); setStatusMsg({ type: 'success', text: 'Menggunakan skrip offline.' });
    } finally { setIsLoading(false); }
  };

  const generateAudio = async () => {
    if (!text.trim()) { setStatusMsg({ type: 'error', text: 'Teks tidak boleh kosong.' }); return; }
    setIsLoading(true); setStatusMsg(null);
    try {
      let blobUrl = ""; let usedVoiceName = "";
      if (activeTab === 'gemini') {
        let mapping = { baseVoice: "Kore", promptContext: "" }; let displayName = "Gemini Custom";
        if (geminiVoiceId.startsWith('clone_')) {
           ...
           Object.values(VOICE_DATABASE_CATEGORIES).forEach(list => { const found = list.find(v => v.id === geminiVoiceId); if (found) displayName = found.name; });
        }
        ...
        const response = await fetchWithRetry(
          ...
          {
            method: "POST", headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ contents: [{ parts: [{ text: finalPrompt }] }], generationConfig: { responseModalities: ["AUDIO"], speechConfig: { voiceConfig: { prebuiltVoiceConfig: { voiceName: mapping.baseVoice } } } } })
          }
        );
        const data = await response.json();
        ...
        if (!inlineData) throw new Error("Audio gagal dibuat.");
        ...
        const response = await fetchWithRetry(
          ...
          {
            method: "POST", headers: { "Content-Type": "application/json", "xi-api-key": elevenLabsApiKey },
      ...
      const newItem: HistoryItem = { id: Date.now().toString(), url: blobUrl, text: text, voice: usedVoiceName, date: new Date().toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }) };
      setHistory(prev => [newItem, ...prev]); setCurrentAudio(newItem); setStatusMsg({ type: 'success', text: 'Audio berhasil dibuat!' });
    } catch (err: any) { setStatusMsg({ type: 'error', text: err.message || "Terjadi kesalahan sistem." }); } finally { setIsLoading(false); }
  };

  const playAudio = (item: HistoryItem) => { setCurrentAudio(item); setTimeout(() => { audioRef.current?.play(); setIsPlaying(true); }, 100); };
  const togglePlayPause = () => { if (audioRef.current?.paused) { audioRef.current.play(); setIsPlaying(true); } else { audioRef.current?.pause(); setIsPlaying(false); } };
  const deleteItem = (id: string) => { setHistory(prev => prev.filter(i => i.id !== id)); if (currentAudio?.id === id) { setCurrentAudio(null); setIsPlaying(false); } };
  const clearAllHistory = () => { if (window.confirm("Hapus semua riwayat audio?")) { setHistory([]); setCurrentAudio(null); } };
  const charCount = text.length; const estTime = `${Math.ceil(charCount / 15)}s`;

  // --- RETURN APP UI ---
  return (
    <div className={`min-h-screen font-sans pb-10 flex flex-col transition-colors duration-500 ${colors.bg} ${colors.text}`}>
      
      {/* HEADER (Tombol Home untuk kembali ke Landing Page) */}
      <header className={`sticky top-0 z-40 backdrop-blur-md border-b transition-colors duration-500 ${isDarkMode ? 'bg-neutral-950/80 border-white/5' : 'bg-white/80 border-blue-500/10'}`}>
        <div className="max-w-6xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {deferredPrompt && (
              <button onClick={handleInstallClick} className={`p-2.5 rounded-xl border transition-all duration-300 ${isDarkMode ? 'border-white/10 hover:bg-white/5' : 'border-slate-200 hover:bg-slate-100 text-slate-600'}`} title="Install App">
                ...
                <div className="space-y-6 animate-in fade-in slide-in-from-left-4 duration-300">
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      <select value={geminiVoiceId} onChange={(e) => setGeminiVoiceId(e.target.value)} className={`w-full border rounded-xl px-4 py-3 appearance-none focus:outline-none focus:border-opacity-100 transition-colors cursor-pointer text-sm ${isDarkMode ? 'bg-neutral-950 border-white/10 text-white focus:border-lime-500' : 'bg-slate-50 border-slate-200 text-slate-900 focus:border-blue-500'}`}>
                         {customClones.length > 0 && (<optgroup label="✨ Rekaman Tersimpan (Lokal)" className={isDarkMode ? "bg-neutral-900 text-lime-400" : "bg-white text-blue-600 font-bold"}>{customClones.map(c => (<option key={c.id} value={c.id} className={isDarkMode ? "text-white" : "text-slate-800 font-normal"}>{c.name}</option>))}</optgroup>)}
                         {Object.entries(VOICE_DATABASE_CATEGORIES).map(([category, voices]) => (<optgroup key={category} label={category} className={isDarkMode ? "bg-neutral-900 text-lime-300" : "bg-white text-blue-600 font-bold"}>{voices.map(v => (<option key={v.id} value={v.id} className={isDarkMode ? "text-white" : "text-slate-800 font-normal"}>{v.name}</option>))}</optgroup>))}
                      </select>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <input type="text" value={styleInstruction} onChange={(e) => setStyleInstruction(e.target.value)} placeholder="Contoh: Sedih, Marah..." className={`w-full border rounded-xl pl-10 pr-4 py-3 focus:outline-none transition-colors ${isDarkMode ? 'bg-neutral-950 border-white/10 text-white placeholder-neutral-600 focus:border-lime-500' : 'bg-slate-50 border-slate-200 text-slate-900 placeholder-slate-400 focus:border-blue-500'}`} />
                    </div>
                  </div>

                  <div className="flex flex-wrap gap-2">
                    {STYLE_PRESETS.map(style => (
                      <button key={style} onClick={() => setStyleInstruction(style)} className={`px-3 py-1.5 rounded-lg text-xs transition-colors border ${styleInstruction === style ? `${isDarkMode ? 'bg-lime-500/20 border-lime-500/50 text-lime-300' : 'bg-blue-500/10 border-blue-500/50 text-blue-600'}` : `${isDarkMode ? 'bg-neutral-800 border-white/5 text-neutral-300' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'}`}`}>
                        {style}
                      </button>
                    ))}
                  </div>
                  ...
                  <div className={`pt-4 border-t space-y-3 ${isDarkMode ? 'border-white/5' : 'border-blue-100'}`}>
                    ...
                    <div className={`rounded-xl p-3 border space-y-3 ${isDarkMode ? 'bg-black/30 border-white/10' : 'bg-slate-50 border-slate-200'}`}>
                       <div className="flex justify-between items-center">
                          <span className={`text-xs font-mono ${colors.textMuted}`}>{isRecording ? `Merekam... ${recordingTime}s` : 'Siap Merekam'}</span>
                          {isRecording && <div className="w-2 h-2 rounded-full bg-red-500 animate-pulse" />}
                       </div>
                       <div className="flex gap-2">
                          <button onClick={isRecording ? stopRecording : startRecording} className={`flex-1 py-2 rounded-lg flex items-center justify-center gap-2 text-xs font-bold transition-all ${isRecording ? 'bg-red-500/20 text-red-400 border border-red-500/50 hover:bg-red-500/30' : `${isDarkMode ? 'bg-neutral-800 text-white border-white/10' : 'bg-white text-slate-700 border-slate-200 shadow-sm'}`}`}>
                            {isRecording ? <Square className="w-3 h-3 fill-current" /> : <Mic className="w-3 h-3" />} {isRecording ? 'Stop' : 'Mulai Rekam'}
                          </button>
                       </div>
                       {recordedUrl && (
                         <div className="space-y-2 animate-in slide-in-from-top-2">
                           ...
                           <div className="flex gap-2">
                             <input value={customCloneName} onChange={(e) => setCustomCloneName(e.target.value)} placeholder="Nama Voice Clone..." className={`flex-1 border rounded px-2 text-xs focus:outline-none ${isDarkMode ? 'bg-neutral-900 border-white/10 text-white' : 'bg-white border-slate-200 text-slate-900'}`} />
                             <button onClick={saveClone} className={`px-3 rounded text-xs font-bold ${isDarkMode ? 'bg-lime-500 text-black' : 'bg-blue-600 text-white'}`}>Simpan</button>
                           </div>
                         </div>
                       )}
                    </div>
                ...
                <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                  <div className={`p-4 rounded-xl border space-y-4 ${isDarkMode ? 'bg-neutral-900 border-white/10' : 'bg-white border-slate-200 shadow-sm'}`}>
                      <div className="flex items-center gap-2 mb-2">
                        <Crown className={`w-4 h-4 ${colors.accent}`} /> <h3 className={`text-sm font-bold ${colors.text}`}>Voice Model</h3>
                      </div>
                      <div className="space-y-3">
                        <div><label className={`text-[10px] uppercase font-bold mb-1 block ${colors.textMuted}`}>Nama Model</label><input value={elevenModelName} onChange={(e) => setElevenModelName(e.target.value)} placeholder="Contoh: My Custom Voice" className={`w-full p-2 text-sm rounded-lg border focus:outline-none ${isDarkMode ? 'bg-black/50 border-white/10 text-white focus:border-lime-500' : 'bg-slate-50 border-slate-200 text-slate-800 focus:border-blue-500'}`} /></div>
                        <div><label className={`text-[10px] uppercase font-bold mb-1 block ${colors.textMuted}`}>ID Model</label><input value={elevenModelId} onChange={(e) => setElevenModelId(e.target.value)} placeholder="Contoh: 21m00Tcm4TlvDq8ikWAM" className={`w-full p-2 text-sm rounded-lg border font-mono focus:outline-none ${isDarkMode ? 'bg-black/50 border-white/10 text-white focus:border-lime-500' : 'bg-slate-50 border-slate-200 text-slate-800 focus:border-blue-500'}`} /></div>
                      </div>
                  </div>
                  {!elevenLabsApiKey && (<div className="flex items-start gap-3 bg-red-500/10 border border-red-500/20 p-3 rounded-xl"><AlertCircle className="text-red-400 w-5 h-5 shrink-0" /><div className="space-y-1"><p className="text-xs font-bold text-red-400">API Key Kosong</p><p className="text-[10px] text-red-300/80">Masukkan API Key ElevenLabs di menu Settings.</p></div></div>)}
                </div>
              )}
            </div>
            {/* Statistik */}
            <div className={`hidden lg:block rounded-3xl border p-6 space-y-4 ${colors.card}`}>
              <div className="flex justify-between items-end"><span className={`text-sm ${colors.textMuted}`}>Jumlah Karakter</span><span className="text-2xl font-bold font-mono">{charCount}</span></div>
              <div className={`h-2 w-full rounded-full overflow-hidden ${isDarkMode ? 'bg-neutral-800' : 'bg-slate-200'}`}><div className={`h-full transition-all duration-500 ${isDarkMode ? 'bg-blue-500' : 'bg-blue-600'}`} style={{ width: `${Math.min((charCount / 500) * 100, 100)}%` }} /></div>
              <div className="flex justify-between items-center pt-2"><span className={`text-xs ${colors.textMuted}`}>Estimasi Durasi</span><span className={`text-sm font-mono ${colors.accent}`}>{estTime}</span></div>
            </div>
          </div>
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between px-2">
                <div className={`flex items-center gap-2 ${colors.textMuted}`}><ListMusic className="w-5 h-5" /><h3 className="font-bold text-sm uppercase tracking-widest">Library & History</h3></div>
                {history.length > 0 && (<button onClick={clearAllHistory} className="text-xs text-red-400 hover:text-red-300 flex items-center gap-1"><Archive className="w-3 h-3" /> Hapus Semua</button>)}
              </div>
              ...
              {currentAudio && (
                <div className={`sticky top-24 z-30 backdrop-blur-md rounded-2xl border p-4 shadow-2xl animate-in slide-in-from-top-4 ${isDarkMode ? 'bg-neutral-800/90 border-lime-500/30' : 'bg-white/90 border-blue-200'}`}>
                  <div className="flex items-center gap-4">
                    <div className={`w-12 h-12 rounded-full flex items-center justify-center border shrink-0 ${isDarkMode ? 'bg-lime-500/20 border-lime-500/30' : 'bg-blue-100 border-blue-200'}`}>
                      {isPlaying ? (<div className="flex gap-0.5 items-end h-5"><div className={`w-1 h-3 animate-[pulse_0.5s_ease-in-out_infinite] ${isDarkMode ? 'bg-lime-400' : 'bg-blue-600'}`}></div><div className={`w-1 h-5 animate-[pulse_0.7s_ease-in-out_infinite] ${isDarkMode ? 'bg-lime-400' : 'bg-blue-600'}`}></div><div className={`w-1 h-2 animate-[pulse_0.4s_ease-in-out_infinite] ${isDarkMode ? 'bg-lime-400' : 'bg-blue-600'}`}></div></div>) : (<Music className={`w-6 h-6 ${isDarkMode ? 'text-lime-400' : 'text-blue-600'}`} />)}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-center mb-1"><h4 className={`font-medium truncate text-sm ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{currentAudio.text}</h4><span className={`text-[10px] border px-1.5 rounded ${isDarkMode ? 'text-lime-400 border-lime-500/20' : 'text-blue-600 border-blue-200 bg-blue-50'}`}>{currentAudio.voice}</span></div>
                      <audio ref={audioRef} src={currentAudio.url} onEnded={() => setIsPlaying(false)} onPause={() => setIsPlaying(false)} onPlay={() => setIsPlaying(true)} controls className={`w-full h-8 ${isDarkMode ? 'accent-lime-500' : 'accent-blue-600'}`} style={{ filter: isDarkMode ? 'hue-rotate(280deg) invert(10%)' : 'none' }} />
                    </div>
                  </div>
                  <div className="flex items-center justify-between mt-3 pl-16">
                     <div className="flex items-center gap-2">
                        <Gauge className={`w-3 h-3 ${colors.textMuted}`} /><select value={playbackSpeed} onChange={(e) => setPlaybackSpeed(parseFloat(e.target.value))} className={`border text-[10px] rounded px-1 py-0.5 focus:outline-none ${isDarkMode ? 'bg-neutral-900 border-white/10 text-white' : 'bg-slate-50 border-slate-200 text-slate-700'}`}><option value="0.5">0.5x (Lambat)</option><option value="1.0">1.0x (Normal)</option><option value="1.25">1.25x (Cepat)</option><option value="1.5">1.5x (Ngebut)</option><option value="2.0">2.0x (Kilat)</option></select>
                     </div>
                     <a href={currentAudio.url} download={generateFilename(currentAudio.text)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-colors ${isDarkMode ? 'bg-lime-500 text-neutral-950 hover:bg-lime-400' : 'bg-blue-600 text-white hover:bg-blue-500'}`}><Download className="w-3 h-3" /> Unduh</a>
                  </div>
                </div>
              )}
              ...
              <div className="space-y-2 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                {history.length === 0 ? (
                  <div className={`text-center py-12 border border-dashed rounded-2xl ${isDarkMode ? 'border-white/10' : 'border-slate-300'}`}><History className={`w-8 h-8 mx-auto mb-2 ${colors.textMuted}`} /><p className={`text-sm ${colors.textMuted}`}>Belum ada riwayat suara.</p></div>
                ) : (
                  history.map((item) => (
                    <div key={item.id} className={`group flex items-center justify-between p-3 rounded-xl border transition-all ${currentAudio?.id === item.id ? `${isDarkMode ? 'bg-neutral-800 border-lime-500/30' : 'bg-blue-50 border-blue-200'}` : `${isDarkMode ? 'bg-neutral-900 border-white/5 hover:bg-neutral-800' : 'bg-white border-slate-200 hover:bg-slate-50'}`}`}>
                      <div className="flex items-center gap-3 min-w-0 flex-1">
                        <button onClick={() => { if (currentAudio?.id === item.id && isPlaying) togglePlayPause(); else playAudio(item); }} className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors ${currentAudio?.id === item.id ? `${isDarkMode ? 'bg-lime-500 text-black' : 'bg-blue-600 text-white'}` : `${isDarkMode ? 'bg-neutral-800 border border-white/10 text-white' : 'bg-white border border-slate-200 text-slate-700'}`}`}>{currentAudio?.id === item.id && isPlaying ? <Pause className="w-3 h-3" /> : <Play className="w-3 h-3 ml-0.5" />}</button>
                        <div className="min-w-0"><p className={`text-sm font-medium truncate pr-4 ${currentAudio?.id === item.id ? colors.accent : colors.text}`}>{item.text}</p><div className={`flex items-center gap-2 text-[10px] ${colors.textMuted}`}><span>{item.date}</span><span>•</span><span>{item.voice}</span></div></div>
                      </div>
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                         <a href={item.url} download={generateFilename(item.text)} className={`p-2 rounded-lg ${isDarkMode ? 'hover:bg-white/10 text-neutral-400 hover:text-white' : 'hover:bg-slate-100 text-slate-400 hover:text-slate-700'}`} title="Unduh"><Download className="w-4 h-4" /></a>
                         <button onClick={() => deleteItem(item.id)} className="p-2 rounded-lg hover:bg-red-500/10 text-neutral-400 hover:text-red-400" title="Hapus"><Trash2 className="w-4 h-4" /></button>
                      </div>
                    </div>
                  ))
                )}
          ...
          <div className={`rounded-2xl border w-full max-w-md relative z-10 p-6 shadow-2xl animate-in zoom-in-95 duration-200 overflow-y-auto max-h-[90vh] ${isDarkMode ? 'bg-neutral-900 border-white/10 text-white' : 'bg-white border-slate-200 text-slate-900'}`}>
            ...
            <div className="space-y-6">
              ...
              <div className={`flex items-center justify-between p-4 rounded-xl border ${isDarkMode ? 'bg-black/30 border-white/5' : 'bg-slate-50 border-slate-100'}`}>
                 ...
                 <button onClick={() => setIsDarkMode(!isDarkMode)} className={`px-3 py-1.5 rounded-lg text-xs font-bold border transition-colors ${isDarkMode ? 'bg-neutral-800 border-white/10 text-white' : 'bg-white border-slate-200 text-slate-800 shadow-sm'}`}>{isDarkMode ? 'Gelap' : 'Terang'}</button>
              </div>
              {/* 3. API KEYS */}
              <div>
                <div className="flex items-center gap-2 mb-2"><Key className={`w-4 h-4 ${colors.textMuted}`} /><h3 className="text-sm font-bold">API Keys Lainnya</h3></div>
                <label className={`text-[10px] uppercase font-bold mb-1 block ${colors.textMuted}`}>ElevenLabs API Key</label>
                <input type="password" value={elevenLabsApiKey} onChange={(e) => setElevenLabsApiKey(e.target.value)} placeholder="sk_..." className={`w-full p-3 rounded-xl border focus:outline-none transition-colors ${isDarkMode ? 'bg-black/50 border-white/10 text-white focus:border-blue-500' : 'bg-slate-50 border-slate-200 text-slate-900 focus:border-blue-500'}`} />
              </div>
              <button onClick={saveSettings} className={`w-full font-medium py-3 rounded-xl flex items-center justify-center gap-2 mt-4 transition-colors text-white ${isDarkMode ? 'bg-green-600 hover:bg-green-500' : 'bg-green-500 hover:bg-green-400 shadow-lg shadow-green-500/20'}`}><Save className="w-4 h-4" />Simpan Semua Pengaturan</button>
            </div>
          </div>
        </div>
      )}
